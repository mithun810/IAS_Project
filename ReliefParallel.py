...
def relief(df, m, label):
    ...
    results = Parallel(n_jobs=num_cores)(delayed(iterator)(featurenumber) for featurenumber in range(df.shape[1]-1))
    :
    ...
    print ranks
    np.savetxt("ReliefParallelRanks.csv", ranks, delimiter=",")
    return ranks

def iterator(featurenumber,df,m,f_weight):
    for j in range(m):
        os.system('clear')
        time=time+1
